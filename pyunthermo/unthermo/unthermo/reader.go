...
func (rf *File) spectrum(sn int, centroided ...bool) (s ms.Spectrum) {
	...
	if len(centroided) > 0 && centroided[0] || scn.Profile.PeakCount <= 0 {
		...
		for i := uint32(0); i < scn.PeakList.Count; i++ {
			s = append(s, ms.Peak{Mz: float64(scn.PeakList.Peaks[i].Mz), I: scn.PeakList.Peaks[i].Abundance})
		}
	} else {
		//convert Hz values into m/z and save the profile peaks
		for k := uint32(0); k < scn.Profile.Nbins+1; k++ {
			s = append(s, ms.Peak{Mz: rf.scanevents[sn-1].Convert(scn.Profile.FirstValue + float64(k)*scn.Profile.Step), I: 0.})
		}
		for i := uint32(0); i < scn.Profile.PeakCount; i++ {
			for j := uint32(0); j < scn.Profile.Chunks[i].Nbins; j++ {
				// apparently the thermofisher dll doesn't seem to use fudge when averaging?? or I'm using it wrong....
				tmpmz := rf.scanevents[sn-1].Convert(scn.Profile.FirstValue+float64(scn.Profile.Chunks[i].Firstbin+j)*scn.Profile.Step) + float64(scn.Profile.Chunks[i].Fudge)
				// s = append(s, ms.Peak{Mz: tmpmz, I: scn.Profile.Chunks[i].Signal[j]})
				s[scn.Profile.Chunks[i].Firstbin+j] = ms.Peak{Mz: tmpmz, I: scn.Profile.Chunks[i].Signal[j]}
			}
		}
	}
	return
...
func (data *ScanEvent) Read(r io.Reader, v Version) {
	if v < 66 {
		...
		binaryread(r, data.Unknown1[10:13]) // CKT: apparently an error -- reading 8 bytes too many, used this as a solution but unclear if reasonable
	}
}
